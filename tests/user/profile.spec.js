...
test.describe('프로필 페이지', () => {
  ...
  test('위시리스트 API가 호출된다', async ({ page }) => {
    ...
    // waitForResponse를 클릭과 동시에 등록 (클릭 이전에 리스너 준비)
    await Promise.all([
      page.waitForResponse(API_WISHLIST, { timeout: 10000 }),
      page.getByRole('button', { name: '위시리스트' }).click(),
    ]);
  ...
  test('구매한 제품 API가 호출되고 렌더링된다', async ({ page }) => {
    ...
    // waitForResponse를 클릭과 동시에 등록
    await Promise.all([
      page.waitForResponse(API_ORDERS, { timeout: 10000 }),
      page.getByRole('button', { name: '구매한 제품' }).click(),
    ]);
