...
// 로그인이 필요 없는 테스트 (별도 describe로 분리)
test.describe('장바구니 페이지 - 비로그인 상태', () => {

  test('로그아웃 상태에서 장바구니 접근 시 alert이 뜬다', async ({ page }) => {
    // 아직 아무 페이지도 방문하지 않았으므로 localStorage는 비어 있음
    // 혹시 모를 토큰 제거를 위해 빈 페이지에서 처리
    await page.goto('/');
